...
public class MsgFrame {
    ...
    public bool CheckFrame(int frame) {
        if (frame < minFrame) { return false; }
        if (frame > maxFrame) { return false; }
        if (!listFrame.ContainsKey(frame)) { return false; }
        return true;
    }
    ...
    public void Update(int frame) {
        ...
        if (!listFrame.ContainsKey(frame)) { return; }
        List<object> msgs = listFrame[frame];
        for (int i = 0; i < msgs.Count; i++) { msgCallback.OnMsg(msgs[i]); }
